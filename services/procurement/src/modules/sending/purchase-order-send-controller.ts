...
import { sendPurchaseOrder } from "./purchase-order-send-service.js";
...
export async function sendPurchaseOrderController(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    const purchaseOrder = await sendPurchaseOrder(
      params.data.id,
      actorIdResult.data,
    );
